...
The current unreleased release train includes `007-standalone-hx-source-independent-cli`, which makes installed `hx` run source-free, ships the two distribution channels (NuGet global tool + Microsoft Store MSIX), removes Velopack, and flips the default version intent to cycle-type-aware (feature → `minor`, bug-fix-only → `patch`); and `008-doti-review-recovery-and-change-context`, which adds Doti cycle **review recovery** (`hx doti cycle refresh-plan`/`refresh --apply-safe` re-binds only the safe-to-reinterpret stamps instead of hand-guessing), a reusable status-rich **change/review context** (`hx doti review-context`, `impact plan --for change-context`, a docs-only gate/Sentrux scope skip), source-scoped Sentrux with an escalation-band diagnostic and an operator- + change-set-fresh-arch-review-gated rebaseline (the baseline is never removed), and an **advisory, never-gating, fully-offline** semantic drift finder (`hx doti drift-candidates`, primary Qwen3-Embedding-0.6B via GGUF/LLamaSharp with a BGE-M3/ONNX fallback, pinned + hash-verified before load).
