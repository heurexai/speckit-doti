...
> The doti engine now owns git-state integrity end to end. A stage transition **stages its own produced artifact** (scoped) so an authored doc is never orphaned in an empty transition commit; and `hx release` runs inside a **compensation ledger** that, on any failure, automatically reverts exactly its own side effects — the tag, the release-root dir (restored to baseline), and the cycle-state bytes — returning a `RollbackReport` instead of leaving a half-released repo. A failed release leaves nothing behind and needs no manual cleanup; concurrent working-tree edits are never touched (it compensates its own effects, not a whole-tree snapshot). A new `hx doti cycle finalize-release` unwedges a cycle stuck at the release stage after a dev→main→CI publish. Shipped in `039-git-transaction-envelope`.
