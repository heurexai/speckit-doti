...
A multi-lens DESIGN review before implementation: TRIAGE the change first, then activate ONLY the lenses it actually touches — a lens whose concern the change does not touch EXITS 'not applicable' and spawns no sub-agent (e.g. a templates/docs-only change skips the architecture-fit, blast-radius, edge-case/failure-mode, security, and testability lenses: 'code is not changing'). Run the applicable lenses (design-soundness, edge-case/failure-mode, data-contract, security, blast-radius, simpler-alternative, testability, fit-with-current-architecture) — each carries a pre-canned 'look for' checklist; if you are capable of sub-agents, run one clean-context sub-agent per lens IN PARALLEL — against the spec/plan/tasks + current code, with the changed-files diff fetched ONCE (the `hx`/Impact planner command, or `git diff` fallback) and injected into every lens so none waste tokens rediscovering scope; each finding carries severity (BLOCKER/HIGH/MEDIUM/LOW) + evidence (`file:line` or spec/plan ref) + a fix. Sentrux/ArchUnitNET are NOT run here — they measure implemented code and are enforced by `gate run` at `/07` (the `cliSurfaceConfinement` and `cliDelegation` ArchUnitNET families plus the Sentrux layer/cycle boundaries). An open BLOCKER in an applicable lens, or a missing applicable lens, blocks `/07-doti-implement`; a correctly-skipped lens does not.
