...
- `SC-077`: A repository generated by `hx new` has the Doti pre-commit hook installed in the resolved Git hooks path, and a normal bare `git commit` is redirected while a Doti-owned workflow-transition, release, or reconcile path (there is no standalone `doti cycle commit` verb — see the "Implemented shape" note above the scenario table) can still create its Git commit through the sanctioned sentinel path.
...
> **Implemented shape (correction, 034):** this section was written against a standalone `doti cycle commit` verb design that was **never built**. No such verb exists — `doti cycle` is `stamp`/`status`/`check`/`refresh-plan`/`refresh`/`review-rebind` (`tools/Hx.Runner.Cli/RunnerCommandFactory.Cycle.cs`), and every sanctioned commit is instead made by the coded path that already validated the preceding proof: a Doti workflow-transition stamp, `hx release`, `hx doti install`/`update` (`DotiReconcileCommit`), or the bug-only release-documentation commit (`hx doti bug release-docs`, `BugReleaseDocCommit`) — each sets the same `PrecommitGuard.SentinelEnvVar` on its own `git commit` subprocess. Read every `commit`/`doti cycle commit` reference below as "the owning sanctioned path" rather than a literal invokable verb; the scenario intent (refuse before Git mutation on a stale/missing/tampered proof, idempotent recovery, no silent bypass) still describes how each real path behaves.
