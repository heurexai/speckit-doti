...
- `FR-003`: Installed `hx` operational commands MUST resolve runtime payload (templates, prerequisite policy, `.doti` assets, vendored tools) relative to the installed executable, identified by a non-source payload descriptor (`payload.manifest.json`) shipped beside it, with an explicit operator override (`HX_PAYLOAD_ROOT`) taking precedence; resolution MUST NOT use the current working directory or a source solution/project file, and MUST fail closed with a structured `Integrity` `CliResult` diagnostic when the payload descriptor is **absent, unparseable, or carries an unsupported `schemaVersion`** (not only when absent). The descriptor MUST be integrity-verified against a **concrete trust root**, not merely parsed: for the normal channel the outer boundary is the signed channel package (NuGet/Store) installed in a read-only location and anchored on `AppContext.BaseDirectory`; the descriptor records a **per-payload-file hash set** and the installed executable carries (embedded, per its own version) the **expected payload-manifest digest**, so `PayloadRoot.Resolve()` recomputes the resolved payload's digest + per-file hashes and **fails closed on mismatch** — a self-consistent but unanchored descriptor (metadata-only) is rejected. This verification MUST run on **every** resolution path, including when `HX_PAYLOAD_ROOT` is used, so the override cannot redirect resolution to a payload that does not match the installed executable's anchored digest; an active override MUST surface a diagnostic.
...
- `FR-031`: For 007, a tier MUST express its gate ladder as a concrete `gates:{ "<step>": "enforced"|"advisory"|"skip" }` map resolved by `GateRunner` (the three curated tiers shipped as concrete files), which is sufficient for FR-029/030. The full layered composition stack (overrides → profile → core with prepend/append/wrap) is **explicitly deferred** beyond 007 — it is premature generality for three fixed tiers and is added only when a real fourth profile must extend a third without forking (justified through Complexity Tracking at that time).
...
- `FR-033`: The tool/release payload MUST ship tool manifests + grammars only (no large binaries); per-RID tool binaries MUST be fetched + hash-verified on demand. The pinned hash MUST derive from a **different trust artifact than the binary** — an upstream-published signed checksum / artifact attestation (verified by the fetch), not a hash self-computed from the same upstream download — and the manifest MUST record the hash provenance; a gate MUST fail if the hash-capture tool version differs from the manifest version. Concurrent fetches of the same binary MUST be safe (download to a unique temp path, atomic rename). **Only a genuine network condition (DNS failure / timeout / unreachable host) MAY degrade to advisory** on the core path; **a malformed manifest, an invalid/blocked URL, a hash/provenance mismatch, or an extract/write failure MUST fail closed** (never advisory) with a structured `CliResult` diagnostic. Because the core install path (`hx new` first smoke) depends on fetched binaries, FR-032/FR-023 define the offline behavior (see FR-032).
...
- `FR-045`: The installed `hx` MUST expose the full doti **workflow command surface source-free** — `gate run`, `architecture test`, `sentrux verify`/`check`, `hygiene scan`, `security scan`, `version calculate`, `doti cycle stamp`/`status`/`check`, `doti render-skills`, `doti install`, `doti payload check`, `doti install-hooks`, and the affected-test planner (`impact plan`) — so a generated or `doti install`-ed repo runs the cycle via the installed `hx` (e.g. `hx gate run`), **not** via `dotnet run --project tools/Hx.Runner.Cli` against vendored source. Today these commands live in the separate `Hx.Runner.Cli`/`Hx.Impact.Cli` executables and `hx new` vendors all 12 runner+impact source projects into every repo via `SourceVendor`; FR-001's source-free rule cannot hold while the repo's workflow path requires that vendored source. The generated repo's `profile.json` command map, `.doti/agent-context.md`, and first smoke MUST be rewritten to the installed-`hx` workflow path, and `SourceVendor`'s tool-source copy removed only once that path exists. (This is the load-bearing dependency of FR-032 / FR-004 / FR-026: "doti install into a repo" must produce a repo whose workflow commands actually exist source-free.)
...
- `SC-004`: No produced in-scope release artifact (global-tool package or MSIX) contains the **tool's own build tree** (`scaffold-dotnet.slnx`, `src/Hx.*`, the tool's `tools/*.csproj`) or a full source-tree archive (`git archive HEAD`); the packaging step fails closed when one is present. The check is an allowlist of expected payload entries over the recursively-walked staged layout and **explicitly allows the legitimate `Hx.Scaffold.Templates` template-pack content** (which by design contains template `.cs`/`.csproj`) and the manifest/grammar payload — a blanket `.csproj`/`src/` ban would false-positive on the required template pack (FR-005).
...
- `SC-011`: Bypass-safe enforcement — a Tier-3 repo with a gate declared enforced but its config deleted/malformed → gate `Fail` (`Integrity_ProfileGateMissingConfig`), not `Skip`; and a proof minted under a silently narrowed/downgraded ladder is refused at transition by the recomputed ladder-coverage binding.
...
- `SC-019`: Source-free workflow surface — in a generated/`doti install`-ed repo with **no vendored `tools/Hx.*` source**, the doti workflow runs via the installed `hx` (`hx gate run`, `hx cycle stamp`, `hx architecture test`, `hx impact plan`, …) and the repo's `profile.json`/agent-context/first-smoke reference that path, not `dotnet run --project tools/Hx.Runner.Cli`; a search finds no generated-repo workflow command that requires vendored runner/impact source.
...
- `tools/Hx.Scaffold.Cli`: remove the Velopack package reference and `VelopackApp.Build().Run()`; route command wiring through a kernel hardening entry so help/errors never reach System.CommandLine defaults; replace `ScaffoldRoot.Resolve` usage in installed commands with the payload resolver; classify installed vs source/developer commands; **wire the `Hx.Runner.Cli` + `Hx.Impact.Cli` workflow command trees into the `hx` root (FR-045) so the installed tool exposes gate/architecture/sentrux/hygiene/security/cycle/impact source-free** (the standalone runner/impact executables remain for the self-host build; `SourceVendor` is retired in favor of generated repos calling installed `hx`).
...
### 2026-06-26 (second arch-review round — Codex panel)

A second independent multi-lens review confirmed 5 further items, all folded in:

- **Workflow surface (FR-045, new):** the installed `hx` must expose the runner/impact/gate/cycle workflow commands source-free; today they live in separate executables and `SourceVendor` copies 12 source projects into every repo, so removing the vendor (as the first round wrote) would leave generated repos with no working doti workflow. T021 rewritten; SC-019 added.
- **Descriptor trust root (FR-003):** "integrity-verified" now names a concrete anchor — an executable-embedded expected payload digest + per-file hashes, verified on every path including `HX_PAYLOAD_ROOT` (a metadata-only self-consistent descriptor is rejected).
- **SC-004 (corrected):** bans the tool's own build tree / full source archive but **allows** the legitimate template-pack `.cs`/`.csproj` content (the blanket `.csproj`/`src/` ban contradicted FR-005).
- **FR-031 (narrowed):** 007 ships the concrete `gates:{step→mode}` map; the composition algebra is deferred (the spec no longer MUST-requires what plan/tasks defer).
- **Contract schema strategy (decided):** the `LocalReleaseResult`/`release.identity.json` Velopack→channel rename is **additive/channel-neutral**, schema stays v1, the frozen `ITG0008` is orphaned-but-kept; a compat test covers it (T004/T028).
- **Package strategy (decided):** since vendored binaries are now fetched not bundled, the global tool is **framework-dependent** (one package, the audience already has the SDK), not RID-specific self-contained (R3/T023).
- Plus: offline degrade restricted to genuine network conditions only (hash/provenance/malformed/extract failures fail closed); Store submission hardened like NuGet publish; `Integrity_ProfileGateMissingConfig` aligned across SC-011/T005/T007.
