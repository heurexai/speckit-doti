...
- **FR-002** [WI1]: A stage transition MUST NOT ORPHAN the leaving stage's produced artifact. The engine fails closed ONLY when the leaving stage declares a `produces` path that, after scoped staging, is absent/orphaned (untracked-and-unstaged, or missing entirely) — with a diagnostic naming it. A `produces` artifact that is already committed and unchanged (e.g. `clarify` re-declaring `specify`'s already-committed spec — the same-artifact `clarify→plan` case) MUST transition normally as a legitimate no-content-change commit; it is NOT fail-closed. This narrows the silent `allowEmpty: !HasStaged` path to exactly the orphan case, without regressing the 031/#42 doc-dance fix.
