...
9. Accept only the `LocalReleaseResult` envelope as proof of the local copy: it must report the release intent, local tag, GitVersion identity, channel package artifacts (the NuGet global-tool package + the Microsoft Store MSIX channel proof), payload checks, install-location proof when produced, config source/path, version directory, latest directory, artifact checksums, or the explicit disabled-copy reason.
...
Expected output: release gate proof plus the `hx release` result showing the local tag, release intent, channel package artifacts (NuGet global tool + Store MSIX channel proof), install-location proof, config source/path, version directory, latest directory, checksums, or the explicit disabled-copy reason.
