...
- **Every requirement testable.** Phrase as `FR-001`, `FR-002`, … ("System MUST …" / "Users MUST be able to …"). The IDs are stable handles that `/06-doti-analyze` maps to tasks for coverage.
...
- **Priority is mode-aware.** The user-story / MVP model fits product code, but not every change. Choose the priority model that fits, and **declare it** in the spec (`/05-doti-tasks` orders the build by it, not always MVP-first):
