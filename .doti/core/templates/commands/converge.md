...
4. Append the genuinely-missing work as new tasks in the right phase of the tasks ledger, mapping each to the requirement it covers (`[covers FR-/SC-]`). Respect the MVP-first phase order and the ordered-task gate — this command never rewrites the ledger for you; you append, and the gate keeps the order honest.
