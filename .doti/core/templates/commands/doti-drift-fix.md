...
5. Reconcile the cycle honestly: `hx doti cycle refresh-plan --target <stage>` shows which stamps can be re-bound (`SafeReinterpret`) vs re-run (`RerunRequired`); `hx doti cycle refresh --target <stage> --apply-safe` re-binds the safe ones; re-run the rest through their `/0N` skill. Then `hx doti cycle check` must pass before you continue.
