...
4. **Organise into MVP-first, gate-enforced phases.** Setup → Foundational (blocking) → User Story N (priority order; P1 = MVP) → Polish. Phases run **sequentially**; each ends at a **Checkpoint** (`gate run` green) before the next begins. Number tasks `T001…` in execution order; mark `[P]` parallelizable (different files, no same-phase dependency) and `[US#]` for story traceability; sequence prerequisites first (contracts/types before consumers) and place the test task before the implementation it covers. (doti keeps phases sequential + gated — it does not run user stories in parallel the way upstream Spec Kit does.)
