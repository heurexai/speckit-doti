...
Sentrux is vendored as the Heurex fork v0.5.12 for declared RIDs with matching C# grammars (win-x64, linux-x64, osx-arm64; binaries are fetched operationally and gitignored); verify/check preserve richer fork diagnostics. The fork's regression `gate` honors `--include-untracked` (passed by the gate's Sentrux step), so a feature's brand-new (not-yet-`git add`-ed) files' structural growth is caught at the gate, not only after they are tracked. Other RIDs remain fail-closed until their assets and grammars are declared. Sentrux measures **production code only** — `.sentruxignore` excludes the repo's `test/` tree (alongside docs, Doti prose, and the `scaffold/templates/` content) so a large, well-structured test fixture is not a false-positive "god file"; the structural barometer reads production architecture. On a violation, `sentrux check` surfaces a **structured offender** (rule id, file, function, line, measured-value-vs-limit where the engine provides them; honest `unknown` with a reason for summary-style rules), not just a count.
...
- `hx doti check-version --repo <path> --json` (report a repo's recorded Doti version from `.doti/payload.json` + its relation to the installed tool: `current`/`outdated`/`ahead`; distinguishes not-a-repo from version-unknown)
- `hx doti scan --root <dir> --json` (recursively discover every Doti-enabled repo under a root + table each one's payload version + relation; read-only, error-tolerant, explicit empty)
- `hx doti update --repo <path> [--force] [--dry-run] --json` (bring a repo's managed Doti assets up to the installed payload and report the before→after version; reconciles in a git worktree — `--dry-run` previews, git required — preserving operator-modified managed assets unless `--force`; constitution/operator-owned content untouched)
- `hx doti update-all --root <dir> [--force] [--dry-run] --json` (batch-update every Doti repo under a root, fail-soft, with per-repo before→after + an updated/already-current/failed summary)
...
- `hx architecture test --repo <path> --json` (run the repo's ArchUnitNET rule families declared in `rules/architecture.json`; speckit-doti currently reports the thin-CLI `cliSurfaceConfinement` and `cliDelegation` families, while Sentrux enforces path/layer/cycle boundaries; fully green on win-x64. On a FAILURE the result names the **violating type(s)** + the rule description — captured from `rule.Evaluate` through the TRX, not just a family pass/fail count — so an operator sees which types broke which family)
...
- `hx tools fetch --repo . [--rid <rid>] [--tool all|gitleaks|sentrux|gitversion] --json` (deterministic, hash-verified provisioning of the vendored tool binaries — gitleaks, sentrux, and gitversion — from their pinned `tools/*/*.version.json` manifests: download → verify `archiveSha256` when present → verify `executableSha256` → write `executablePath` (set executable on Unix, `0755`, so the hash-verified binary runs on Linux/macOS); fail-closed on mismatch (integrity/`tool-*-hash-mismatch`); a RID with no asset is reported cleanly (validation/`tool-asset-unavailable`), never thrown. Fetch-if-missing; `new` runs it best-effort after vendoring so generated repos carry a working gitversion and release tooling. The offline gate verifies the already-present binaries/packages — this fetch is a provisioning step, not part of the gate)
...
- `hx doti cycle refresh-plan --target <stage> --repo . --json` + `doti cycle refresh --target <stage> --apply-safe --repo . --json` (review recovery — `refresh-plan` is the read-only recovery plan: per transitively-required stage it classifies the existing stamp `SafeReinterpret` (binding-format migration → re-bindable), `ReBindContentEqual` (content-equal / edge-only — the stage's OWN artifact is unchanged under canonical hashing, every producing upstream is fresh, and every shared prerequisite is byte-identical, so only the binding edge moved → auto-rebindable; a real content change can NEVER reach this), `RerunRequired` (the artifact's content genuinely changed → re-run that stage; review-kind and change-set-bound stages are always here), `inserted-stage` (the current graph requires a stage cycle-state never stamped → run its `/NN` skill), or `NotBound`; `refresh --apply-safe` re-stamps BOTH the `SafeReinterpret` AND `ReBindContentEqual` stages (re-deriving the plan after each so a content-equal chain settles in one pass) and refuses the rest (`cycle-refresh-rerun-required`/`cycle-refresh-not-bound`). Re-running the ONE genuinely-changed stage now AUTO-CASCADES the content-equal rebind to its content-unchanged dependents — the hand-re-stamp treadmill is gone — so a stale cycle recovers without hand-guessing a stamp)
- `hx doti review-context --base <ref> --repo . --json` (project the change set into arch-review lens applicability — `data.review.*`: the change categories (`doti-prose`/`generated-template`/`runtime-code`/`docs-only`/`contract`) + which lenses apply vs skip, so `/04` reads triage as data and a docs-only diff has a machine-checkable category set. It ALSO carries `data.constitution` — the fresh **§2** the arch-review evaluates against, FR-008)
- `hx doti constitution [--section §2|full] --repo . --json` (emit the project constitution — the **§2** project declarations by default (domain, tech stack, coding style, security, performance), `--section full` for §1+§2. The codified carrier for the fresh-context injection at `/03-doti-plan` (and composed into `review-context` for `/04`), and an on-demand check of what the stages evaluate against. Surface-and-proceed when absent — §1 stays gate-enforced; author/amend §2 with `/doti-constitution`)
- `hx doti drift-candidates --base <ref> [--model-root <dir>] --repo . --json` (advisory semantic drift finder, **never gating**, **on-demand at any cycle stage** — embeds changed code vs reference prose on a local CPU model (primary Qwen3-Embedding-0.6B via GGUF/LLamaSharp, fallback BGE-M3 via ONNX; pinned + hash-verified, fully offline), tuned for **.NET** findings (member-level C# chunking + a code-aware instruction + .NET-calibrated thresholds). **Match-type guidance — reach for it ONLY for PARAPHRASE code↔docs drift** (a renamed/changed symbol vs a stale doc/skill/help summary that no longer matches in words); for **IDs / markers / exact strings use `grep`** (deterministic + exhaustive), and for **contradictions** (a doc asserting the OPPOSITE of the code) use **reasoning**, never cosine (embeddings rate opposites as *similar*). It is **code↔docs-only** (NOT wired into `converge`/`analyze`). Skips cleanly when no model is provisioned; an empty candidate list is NOT a clean-bill signal — only the deterministic `/08` axes clear drift. The model root resolves from `hx.config.json` `llmModelRoot` (wins), else `HEUREX_LLM_ROOT`, else the finder skips)
...
- `hx gate run --repo . --profile <auto|advisory|normal|release> --stream --json` (stream NDJSON ladder-phase events live — each carries the step outcome + per-step duration — then the final envelope, which additionally carries a `GateRunResult.Trace`: the operator-facing visibility trace — effective scope (docs-only vs code), the effective affected-test mode (`full`/`partial`/`none`), a two-tier change-set summary (basic file/line counts on every run; classes-touched + selected/total test projects only at the implement-stage code gate), the per-step ladder with durations, and total elapsed. On a human TTY the same trace renders through the shared kernel renderer — outcome-aware progress bars + a bounded trace panel (a capped summary, never a dump) — so an operator sees scope, per-step pass/skip/fail + reason, and timing without `--json`. When a structural step (`architecture-test` / `sentrux-*`) FAILS, the ladder also renders the offender detail — the ArchUnit violating types / the Sentrux offending file·function·line — capped (`+N more`) + deterministically ordered. The trace (including the structural offenders) is review/telemetry context carried on the ENVELOPE only — never a proof-hash input, so the persisted affected-test proof + gate-proof digest are byte-unchanged by it)
