...
8. **Codified commit (the only path).** Commit through `Hx.Runner.Cli doti cycle commit --message <m>` — it re-verifies the prerequisite chain (a fresh drift-review + the task-hash, via `cycle check`), a fresh passing gate proof (run `gate run` first so it is persisted), and a clean staged scope, then commits; it refuses otherwise. **Do not `git commit` by hand** — the insurance pre-commit hook (`doti install-hooks`) redirects you here.
