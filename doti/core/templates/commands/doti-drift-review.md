...
5. Run `gate run --profile normal --repo .` and treat any gate failure as blocking before handing off to commit.
6. On a clean review, stamp the stage (`doti cycle stamp --stage drift-review`) so the commit chokepoint can verify it, then confirm commit-readiness with `doti cycle check --stage commit` (fail-closed: every prerequisite stamped + fresh).
