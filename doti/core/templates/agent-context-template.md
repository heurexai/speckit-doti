...
- `dotnet run --project tools/Hx.Runner.Cli -- architecture test --repo <path> --json` (run a generated repo's ArchUnitNET rule families; per-`[Fact]` proof + the eight families from `rules/architecture.json`; fully green on win-x64)
